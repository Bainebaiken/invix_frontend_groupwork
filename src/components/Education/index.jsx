...
function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://invix-backend-group-work.onrender.com/api/v1/articles/get_education');
        if (!response.ok) {
          throw new Error('Failed to fetch data');
        }
        const data = await response.json();
        // const technologyPosts = data.filter(article => article.category === '3');
        const technologyPosts = data.filter(article => article.category === 'Education');
